...
(
    ...
    output  reg             touch_valid  , //触摸标志
    output  reg [15:0]      tp_x_coord   , //X方向触摸点的坐标
    output  reg [15:0]      tp_y_coord   , //Y方向触摸点的坐标                           
    output  reg             touch_rst_n  , //触摸屏复位
    input                   touch_int_in , //INT输入信号
    output  reg             touch_int_dir, //INT方向控制信号
    output  reg             touch_int_out  //INT输出信号
 );
...
localparam st_coord_handle  = 7'b100_0000;//针对不对尺寸的触摸的坐标数据进行处理
...
always @ (posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        case(next_state)
            ...
            st_init : begin
                cnt_1us_en <= 1'b1;
                if(cnt_1us_cnt < 10_000)             //延时10ms
                    touch_rst_n <= 1'b0;             //开始复位
                else if(cnt_1us_cnt == 10_000)
                    touch_rst_n <= 1'b1;             //结束复位
                else if(cnt_1us_cnt == 60_000) begin //再次延时50ms(60_000-10_000)
